                            ...
                            <tbody>
                                @foreach ($kategoriBuku as $kategoriBuku)
                                    <tr class="odd:bg-white even:bg-gray-50 border-b">
                                        ...
                                        <td class="px-6 py-4">
                                            <button data-modal-target="modalEdit{{ $kategoriBuku->id }}" data-modal-toggle="modalEdit{{ $kategoriBuku->id }}" type="button" class="text-white bg-blue-500 hover:bg-blue-600 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center me-2">
                                                <svg class="w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
                                                    <path d="M12.687 14.408a3.01 3.01 0 0 1-1.533.821l-3.566.713a3 3 0 0 1-3.53-3.53l.713-3.566a3.01 3.01 0 0 1 .821-1.533L10.905 2H2.167A2.169 2.169 0 0 0 0 4.167v11.666A2.169 2.169 0 0 0 2.167 18h11.666A2.169 2.169 0 0 0 16 15.833V11.1l-3.313 3.308Zm5.53-9.065.546-.546a2.518 2.518 0 0 0 0-3.56 2.576 2.576 0 0 0-3.559 0l-.547.547 3.56 3.56Z"/>
                                                    <path d="M13.243 3.2 7.359 9.081a.5.5 0 0 0-.136.256L6.51 12.9a.5.5 0 0 0 .59.59l3.566-.713a.5.5 0 0 0 .255-.136L16.8 6.757 13.243 3.2Z"/>
                                                </svg>
                                            <form action="{{ route('kategoriBuku.destroy', $kategoriBuku->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <div class="relative p-4 w-full max-w-md max-h-full">
                                            ...
                                            <div class="relative p-4 bg-white rounded-lg shadow">
                                                ...
                                                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                                                    ...
                                                    <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-toggle="modalEdit{{ $kategoriBuku->id }}">
                                                ...
                                                <div class="p-4 md:p-5">
                                                    <form action="{{ route('kategoriBuku.update' , $kategoriBuku->id) }}" method="POST" enctype="multipart/form-data">
                                                        @csrf
                                                        @method('PUT')
                                                        <div class="grid gap-4 mb-4 grid-cols-2">
                                                            <div class="col-span-2">
                                                                <label for="buku_id" class="block mb-2 text-sm font-medium text-gray-900">Buku</label>
...
<div id="modalTambah" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <!-- Modal body -->
            <form action="{{ route('kategoriBuku.store') }}" method="POST" enctype="multipart/form-data" class="p-4 md:p-5">
                @csrf
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2">
                        <label for="buku_id" class="block mb-2 text-sm font-medium text-gray-900">Buku</label>
